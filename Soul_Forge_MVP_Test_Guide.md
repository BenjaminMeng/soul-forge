...
### 1.2 确认你的 OpenClaw 工作空间路径

OpenClaw workspace 路径确认优先级：
1. 启动时 `--workspace` 参数指定的目录
2. `openclaw.json` 中配置的目录
3. 默认路径 `~/.openclaw/workspace/`

**记录你的工作空间路径，后续步骤用 `{WORKSPACE}` 代替。**
...
| 源文件 | 部署目标 | 操作 |
|--------|---------|------|
| `skills\soul-forge\SKILL.md` | `{WORKSPACE}\skills\soul-forge\SKILL.md` | 复制（创建目录） |
| `hooks\soul-forge-bootstrap\HOOK.md` | `{WORKSPACE}\hooks\soul-forge-bootstrap\HOOK.md` | 复制（创建目录） |
| `hooks\soul-forge-bootstrap\handler.js` | `{WORKSPACE}\hooks\soul-forge-bootstrap\handler.js` | 复制 |
| `.soul_forge\config.json` | `{WORKSPACE}\.soul_forge\config.json` | 复制（创建目录） |
| `.soul_forge\memory.md` | `{WORKSPACE}\.soul_forge\memory.md` | 复制 |
| `HEARTBEAT_SEGMENT.md` | 手动追加到 `{WORKSPACE}\HEARTBEAT.md` | 见 2.3 步骤 |
...
```cmd
set WORKSPACE={你的工作空间路径}
set SOURCE=d:\Coding\OpenClaw\OpenClaw_Indiviual_SOUL.md

rem 创建目录
mkdir "%WORKSPACE%\skills\soul-forge" 2>nul
mkdir "%WORKSPACE%\hooks\soul-forge-bootstrap" 2>nul
mkdir "%WORKSPACE%\.soul_forge" 2>nul

rem 复制 Skill
copy "%SOURCE%\skills\soul-forge\SKILL.md" "%WORKSPACE%\skills\soul-forge\SKILL.md"

rem 复制 Hook
copy "%SOURCE%\hooks\soul-forge-bootstrap\HOOK.md" "%WORKSPACE%\hooks\soul-forge-bootstrap\HOOK.md"
copy "%SOURCE%\hooks\soul-forge-bootstrap\handler.js" "%WORKSPACE%\hooks\soul-forge-bootstrap\handler.js"

rem 复制运行时数据（仅当目标不存在时）
if not exist "%WORKSPACE%\.soul_forge\config.json" (
  copy "%SOURCE%\.soul_forge\config.json" "%WORKSPACE%\.soul_forge\config.json"
)
if not exist "%WORKSPACE%\.soul_forge\memory.md" (
  copy "%SOURCE%\.soul_forge\memory.md" "%WORKSPACE%\.soul_forge\memory.md"
)
```
...
```
[ ] {WORKSPACE}\skills\soul-forge\SKILL.md 存在
[ ] {WORKSPACE}\hooks\soul-forge-bootstrap\HOOK.md 存在
[ ] {WORKSPACE}\hooks\soul-forge-bootstrap\handler.js 存在
...
- 确认文件在正确路径 `{WORKSPACE}\skills\soul-forge\SKILL.md`
