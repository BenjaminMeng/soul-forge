...
>
> A local `telemetry.json` file is also generated with aggregate metrics (session count, mood trends, etc.) — it contains **no conversation content**. Soul Forge always sends a small anonymous ping (version, calibration status, session count, DISC type — no conversation content) to help track adoption. You can optionally enable enhanced anonymous telemetry with `/soul-forge telemetry enable`, which adds mood trends and modifier details. Enhanced telemetry is **opt-in only** and can be disabled at any time.
>
> Sound good?
...
>
> 本地还会生成一个 `telemetry.json` 文件，包含汇总指标（会话次数、情绪趋势等）——**不包含任何对话内容**。Soul Forge 会发送一个小型匿名 ping（版本号、校准状态、会话次数、DISC 类型——不含对话内容）以帮助统计使用情况。你可以通过 `/soul-forge telemetry enable` 启用增强版匿名遥测，额外发送情绪趋势和调节器详情。增强遥测**始终是用户主动选择（opt-in）**，可随时关闭。
>
> 可以开始吗？
...
false
```
2. **⚠️ MANDATORY — Reply with EXACTLY this sentence, word for word. Do NOT paraphrase, summarize, or add any other content:**
   - EN: "Enhanced telemetry disabled. Only minimal anonymous data will continue to be sent."
   - ZH: "增强遥测已关闭。仅继续发送最低限度的匿名数据。"
...
**Behavior:**
**⚠️ MANDATORY — Reply using EXACTLY this one-line format. Do NOT add personality info, modifier details, observation history, or any other content. Fill in the bracketed values only:**
- EN: "Telemetry: Minimal always active. Enhanced: [enabled/disabled]. Anonymous ID: [id or 'not set']. Minimal data: version, status, DISC type, session count. Enhanced data adds: modifiers, mood trends."
- ZH: "遥测：最低限度始终启用。增强遥测：[已启用/已关闭]。匿名 ID：[id 或 '未设置']。最低限度数据：版本、状态、DISC 类型、会话数。增强遥测额外包含：调节器、情绪趋势。"
