...
11. [Smart Merge 写入机制](#11-smart-merge-写入机制)
12. [版本管理](#12-版本管理)
13. [安全与边界](#13-安全与边界)
...
| **三重保障观察** | 人格信号收集不依赖单一机制，三层冗余确保数据持续积累 | v2.2 架构升级推演 |
...
| **Verbosity (话语量)** | 回复的长度和详细程度 | 0-3 (极简/简洁/正常/详细) | 2 |
| **Proactivity (主动性)** | 是否主动提出建议和发现 | 0-3 (只答问题/偶尔建议/适度建议/积极主动) | 1 |
| **Challenge (挑战度)** | 是否挑战用户观点、戏谑式互动 | 0-3 (从不/温和/中等/高) | 0 |
...
> **MVP 范围：** MVP 不含修饰符问卷（仅 8 题 DISC），修饰符全部设为默认值（各维度=1）。
> **Phase 2：** Heartbeat 观察 + Skill 校准逐步发现修饰符偏好。
...
**成本为零**（纯话术），但准确率从问卷本身的 ~70% 提升到用户确认后的 ~85%+。
...
| `soul-forge-collector` | Hook | /new 时（自动） | 补充提取（从原始 transcript） | 中（用户可能不用 /new） | Phase 2 |
...
- SOUL.md + IDENTITY.md 生成/更新（模板组装 + overlay merge）
...
| 命令 | 功能 | 数据处理 | 阶段 |
|------|------|---------|------|
| `/soul-forge` | 首次问卷 + 校准 | 创建全部文件 | MVP |
| `/soul-forge calibrate` | 修饰符校准 | 读 memory.md → 更新 config.json + SOUL.md | MVP |
...
| `/soul-forge reset` | 恢复默认 | 从 SOUL_INIT.md 恢复，清空 .soul_forge/（.soul_history 保留） | MVP |
...
/soul-forge recalibrate（重新校准）：
  ...
  8. changelog.json 追加 trigger: "recalibrate"

/soul-forge reset（恢复默认）：
  1. 从 .soul_history/SOUL_INIT.md 恢复 SOUL.md
  2. 从 .soul_history/IDENTITY_INIT.md 恢复 IDENTITY.md
  3. 删除 .soul_forge/config.json + memory.md
  4. .soul_history/ 保留（快照永不删除）
  5. 相当于 Soul Forge 从未运行过
```
...
✅ .soul_forge/config.json 存在且 status="calibrated"
...
1. **读取状态**：解析 `.soul_forge/config.json` 获取当前 DISC 类型、修饰符值、状态
2. **读取观察**：解析 `.soul_forge/memory.md` 提取最近 20 条 active 观察
3. **生成注入文件**：动态生成校准上下文，通过 `context.bootstrapFiles` 注入系统提示
...
4. **检查 HEARTBEAT.md**：确认 Soul Forge 标记段存在，如被删除则自动重新追加
5. **首次提醒**：如果 config.json `status = "fresh"`，在注入上下文中提示 Agent 建议用户运行 `/soul-forge`
...
- If status = "paused": skip all checks, do nothing
- If status = "calibrated":
  ...
  If ANY answer is not "neutral/just right":
    Append to .soul_forge/memory.md using this format:
    ## YYYY-MM-DD HH:MM
    - **type**: style|emotion|boundary|decision
...
- Check .soul_forge/memory.md: if 5+ active observations point same direction
  for any modifier → suggest user run /soul-forge calibrate (max once per day)
<!-- SOUL_FORGE_END -->
```
...
### 7.6 soul-forge-collector Hook（Phase 2）
...
4. Skill 正式更新
   ...
   → Skill BDI 决策 → 更新 config.json + SOUL.md 修饰符值 → 已结晶观察归档
```
...
Action  ← 用户确认后执行：
          1. 更新 config.json 中的 modifiers 值
          2. 更新 SOUL.md 对应段落
          3. memory.md 中相关条目 status → archived
          4. config.json 追加 calibration_history 条目
          5. .soul_history/ 保存快照
```
...
- Skill 以"AI 学习人类表达方式"为伪装，在对话中偶尔给出两种表达让用户选择
...
> **MVP 范围：** MVP 实现 Phase 1（安装后首问 + Heartbeat 结构化检查 + SOUL.md 身份层被动观察）。修饰符使用默认值，不主动试探。
> **Phase 2：** 实现完整三阶段发现策略 + 定期主动询问用户。

### 8.5 结晶与归档

**结晶触发条件（MVP）：**
- 同一 modifier_hint 方向的 active 观察 ≥ 5 条
- 由 Heartbeat 检测并建议用户运行 `/soul-forge calibrate`（每天最多建议一次）

**信号质量加权（Phase 2 实现）：**

MVP 使用简单计数（≥5 条），Phase 2 引入加权系统提高校准精度：
...
> ```
> hooks/
>   └── soul-forge-collector/     # Collector Hook
>       ├── HOOK.md
>       └── handler.ts
>
> .soul_forge/
>   └── memory_archive.md         # 归档的历史观察
> ```
...
| .soul_forge/config.json | Skill（校准时） | 状态 + DISC 结果 + 修饰符 | 读写 |
...
{
  "version": 1,
  "status": "fresh | calibrated | paused",
  "disc": {
    "primary": "S",
    "secondary": "C",
...
| 字段 | 用途 | 写入时机 |
|---|---|---|
| status | 区分首次安装(fresh)、已校准(calibrated)、暂停(paused) | 安装时=fresh，问卷完成后=calibrated，用户暂停时=paused |
...
### 9.4 memory.md 与 config.json 职责划分
...
| **写入者** | 只有 Skill | Heartbeat / Agent / Collector Hook |
| **读取者** | Bootstrap Hook / Skill / Heartbeat | Bootstrap Hook / Skill |
| **格式** | JSON | Markdown |
...
│                                                                  │
│  用户安装 Soul Forge                                             │
│  → 创建 .soul_forge/config.json (status: fresh)                 │
...
│  → 模板组装 → overlay merge → 写入 SOUL.md + IDENTITY.md        │
│  → config.json status → calibrated                              │
└──────────────────────┬──────────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────────┐
...
│                                                                  │
│  每次启动：                                                       │
│    Bootstrap Hook → 读 config + memory → 注入校准上下文           │
│    → Agent 获得观察指令 + 当前参数 + 最近观察                      │
│                                                                  │
│  对话中：                                                         │
...
│                      持续校准阶段                                 │
│                                                                  │
│  Heartbeat 检测：同方向观察 ≥ 5 条                                │
│  → 建议用户运行 /soul-forge calibrate (每天最多 1 次)             │
│  → Skill BDI 决策 → 自然对话式确认                               │
│  → 用户确认 → 更新 config.json + SOUL.md                        │
│  → memory.md 相关条目归档                                        │
│  → 保存快照到 .soul_history/                                     │
└─────────────────────────────────────────────────────────────────┘
```
...
3. config.json 不存在 + SOUL.md 末尾有标记 → 状态 3
4. config.json 不存在 + SOUL.md 匹配原始模板 → 状态 1（全新）
5. config.json 不存在 + SOUL.md 不匹配模板 → 状态 2（已定制）
```
...
**MVP 跳过的部分（Phase 2 实现）：**
- 内容比对（normalize + 模板匹配）
- 参数推断（从已有内容推断修饰符）
- 融合选项（保留部分已有内容）
- user_customizations.json 生成
...
3. Parse            → 按 H2 标题 (## xxx) 切分为字典
4. Overlay          → 叠加写入（非替换）
   - Core Truths: 保留 OpenClaw 原文 5 段 + 追加 Soul Forge 内容
   - Vibe: 完全替换为 Soul Forge 内容（唯一的完全替换段）
   - Boundaries: 保留 OpenClaw 原文 4 条 + 追加 Soul Forge 内容
   - Continuity: 不触碰
5. Preserve         → 原封保留所有未管辖的段落（如 Tool Definitions）
6. Render           → 重组文档并写入
7. Changelog        → 写入 changelog.json 记录变更
8. Marker           → 在 SOUL.md 末尾写入标记 [//]: # (soul-forge:...)
```
...
>
> 原 v2 的 merge 逻辑由 Python 脚本实现。v3 中全部由 Skill 指令引导 Agent 完成：
> - **SKILL.md** 包含组装规则的自然语言描述
> - **Agent** 使用内置文件读写工具执行实际操作
> - **handler.ts（Bootstrap Hook）** 负责 Markdown 解析和结构化数据提取
>
> 这简化了安装依赖（不需要 Python），但对 SKILL.md 的指令质量要求更高。
...
```json
{
  "versions": [
    {
      "version": 1,
      "timestamp": "2026-02-12T19:30:22Z",
      ...
      "snapshot_file": "SOUL_20260212_193022.md"
    },
    {
      "version": 2,
      ...
      "snapshot_file": "SOUL_20260220_100000.md"
    }
  ]
}
```
...
> **Phase 2：** Skill 内回滚命令（`/soul-forge rollback`）。
...
**目标：** 安装流程自动化 + 修饰符自动发现 + 分发

**新增组件：**
- soul-forge-collector Hook
- 安装脚本（install.sh / install.ps1）
- ClawHub 发布包

**新增功能：**
- 完整 Pre-flight Check（内容比对 + 参数推断 + 融合选项）
- 修饰符三阶段发现（被动观察 → 主动校准 → 成熟）
- memory.md 归档清理
- Skill 内回滚命令
...
- 1 个 Hook（soul-forge-collector）— Phase 2
- HEARTBEAT.md 追加段
- .soul_forge/ 数据目录
...
| 18 | SOUL.md 更新方式 | overlay merge（锚点式 H2 段叠加） |
| 19 | 权重系统 | 不使用。通过文本位置和措辞强度控制 |
| 20 | SOUL.md 体积 | < 4KB |
| 21 | 版本管理 | .soul_history/ + changelog.json |
...
| 41 | Collector Hook | Phase 2 实现，补充机制 |
...
| 56 | memory.md 归档 | MVP 不实现，Phase 2 实现（阈值：200 条或 100KB） |
...
| 62 | 重新校准 vs 恢复默认 vs 暂停 | recalibrate 保留 memory + 修饰符、重跑 DISC；reset 从快照恢复、清空数据；pause 暂停观察 |
| 63 | 安装后偏好首问 | 校准完成时主动问一次偏好（如回复长度），作为高权重首条观察 |
| 64 | 信号质量加权 | Phase 2 引入加权（直接反馈=3，行为推断=1，无意义=0），阈值 ≥8 |
| 65 | 交付验证检查清单 | Skill 执行完毕前自检 5 项（SOUL.md/IDENTITY.md/config.json/memory.md/HEARTBEAT.md）|
| 66 | config.json paused 状态 | 新增 paused 状态值，Heartbeat 检测到时跳过所有检查 |
...
| M5 | 编写 HEARTBEAT.md Soul Forge 段（结构化 6 问检查清单 + paused 检查） | 无 | 待开始 |
| M6 | 定义 config.json 初始结构（含 paused 状态） | 无 | 待开始 |
...
| ID | 事项 |
|---|---|
| P1 | soul-forge-collector Hook 设计与实现 |
| P2 | 完整 Pre-flight Check（内容比对 + 融合） |
| P3 | 修饰符三阶段发现策略实现 |
| P4 | memory.md 归档清理机制 |
| P5 | Skill 内回滚命令 |
| P6 | 安装脚本（install.sh / install.ps1） |
| P7 | ClawHub 发布包制作 |
| P8 | MVP 测试计划（LLM 模型 × 观察协议执行率） |
...
*Soul Forge v3 — 用 DISC 定义角色，用 Heartbeat 持续观察，用 BDI 理性校准，用数据驱动进化。*
